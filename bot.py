import time, threading, json, os, urllib.request, urllib.parse, random
...
def http_post(url, data):
    ...
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", "Content-Type": "application/x-www-form-urlencoded", "Origin": "https://steamcommunity.com", "Referer": "https://steamcommunity.com/login/"}
...
def steam_login():
    try:
        ...
        rsa = json.loads(r)
        resp = http_post("https://steamcommunity.com/login/dologin", {"username": STEAM_LOGIN, "password": STEAM_PASS, "emailauth": "", "captchagid": "-1", "captcha_text": "", "emailsteamid": "", "rsatimestamp": rsa.get("timestamp", 0), "remember_login": "true"})
        result = json.loads(resp)
        print(f"LOGIN: {result}")
        if result.get("success"): return True
        if result.get("message"): print(f"ERR: {result['message']}")
        return False
...
async def btn(update, context):
    ...
    if d == 'on':
        if not steam_login(): await q.edit_message_text("❌ Не удалось зайти в Steam. Смотри логи."); return
        running = True
        threading.Thread(target=farm_loop, daemon=True).start()
        await q.edit_message_text("🚀 Фарм запущен! Dota 2 + CS2 + TF2.")
    elif d == 'off': running = False; await q.edit_message_text(f"⏹ Стоп. Часов: {stats['hours']}")
    elif d == 'st': await q.edit_message_text(f"📊 Статус: {'🟢' if running else '🔴'}\nЧасов: {stats['hours']}")
